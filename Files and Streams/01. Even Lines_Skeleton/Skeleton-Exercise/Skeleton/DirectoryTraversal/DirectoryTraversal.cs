namespace DirectoryTraversal
{
    ...
    public class DirectoryTraversal
    {
        ...
        public static string TraverseDirectory(string inputFolderPath)
        {
            ...
            var orderedExtensionFiles = extensionFiles.OrderByDescending(ef => ef.Value.Count);
            StringBuilder sb = new StringBuilder();

            foreach (var extensionsFiles in orderedExtensionFiles)
            {
                sb.AppendLine(extensionsFiles.Key);
                var orderedFiles = extensionsFiles.Value.OrderByDescending(f => f.Length);
